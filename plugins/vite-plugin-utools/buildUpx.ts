...
const validatePluginOptions = (options: PluginOptions) => {
  requiredKeys.forEach((key) => {
    if (!options[key])
      throw new Error(
        colors.red(`plugin ${key} is required,see: ${colors.bold(DOC_URL)}`)
      );
  });
};
...
const prepareOutDir = async (
  buildOptions: NestedRequired<BuildUpxOptions>,
  ...
  return pathResolve(
    buildOptions.outDir,
    generateOutName(buildOptions.outName, pluginOptions)
  );
};
...
export const buildUpx = async (
  ...
  try {
    const pluginOptions = formatPluginOptions(
      getPluginOptions(buildOptions.pluginPath),
      !!preload
    );
    logger.info(
      `${colors.green("plugin.json for building upx:")}\n${JSON.stringify(
        pluginOptions,
        null,
        2
      )}`
