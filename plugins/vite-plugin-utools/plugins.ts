...
export const preloadPlugin = (
  preloadOptions: RequiredOptions["preload"]
): Plugin => {
  if (!preloadOptions)
    return {
  ...
  return {
    ...
    config: (userConfig) => ({
      base:
        isUndef(userConfig.base) || userConfig.base === "/"
          ? ""
          : userConfig.base,
      build: {
        rollupOptions: {
          ...
          output: {
            entryFileNames: ({ facadeModuleId: id }) =>
              filter(id)
                ? "preload.js"
                : `${userConfig.build?.assetsDir || "assets"}/[name].js`,
          },
        },
      },
    }),
...
export const apiExternalPlugin = (
  apiExternal: RequiredOptions["external"]
): Plugin => {
  return {
    ...
    transform: (code, id) =>
      !transformFilter(id)
        ? code
        : transformExternal(code, (sourcePath) =>
            sourcePath === apiExternal ? "window.utools" : void 0
          ),
  };
};
